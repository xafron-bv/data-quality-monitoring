...
class JsonRulesValidator(ValidatorInterface):
    ...
    def _fail(self, error_type: Any, probability: float, details: Dict[str, Any]) -> ValidationError:
        return ValidationError(error_type=error_type, probability=probability, details=details)

    def _ensure_string(self, value: Any, allow_numeric_cast: bool) -> Tuple[Optional[ValidationError], Any]:
    ...
    def _handle_not_empty(self, value: Any, rule: Dict[str, Any]) -> Optional[ValidationError]:
        if pd.isna(value) or (isinstance(value, str) and not value.strip()) or value == "":
            return self._fail(rule.get("error_type", self.GenericErrorCode.MISSING_VALUE), rule.get("probability", 1.0), rule.get("details", {}))
        return None
    ...
    def _handle_type_is_string(self, value: Any, rule: Dict[str, Any]) -> Tuple[Optional[ValidationError], Any]:
        ...
        return self._ensure_string(value, allow_cast)
    ...
    def _handle_no_leading_trailing_whitespace(self, value: str, rule: Dict[str, Any]) -> Optional[ValidationError]:
        if value != value.strip():
            ...
            return self._fail(rule.get("error_type", self.GenericErrorCode.INVALID_FORMAT), rule.get("probability", 0.95), details)
        return None

    def _handle_no_line_breaks(self, value: str, rule: Dict[str, Any]) -> Optional[ValidationError]:
        if "\n" in value or "\r" in value:
            return self._fail(rule.get("error_type", self.GenericErrorCode.INVALID_FORMAT), rule.get("probability", 0.9), rule.get("details", {}))
        return None
    ...
    def _handle_regex_required(self, value: str, rule: Dict[str, Any]) -> Optional[ValidationError]:
        ...
        if pattern and not re.search(pattern, value):
            return self._fail(rule.get("error_type", self.GenericErrorCode.INVALID_FORMAT), rule.get("probability", 0.9), rule.get("details", {"value": value}))
        return None
    ...
    def _handle_must_match_one_of(self, value: str, rule: Dict[str, Any]) -> Optional[ValidationError]:
        ...
        if not any(re.match(p, value) for p in patterns):
            return self._fail(rule.get("error_type", self.GenericErrorCode.INVALID_FORMAT), rule.get("probability", 0.8), rule.get("details", {"invalid_value": value}))
        return None
    ...
    def _handle_tokens_not_empty(self, value: str, rule: Dict[str, Any]) -> Optional[ValidationError]:
        ...
        if not tokens:
            return self._fail(rule.get("error_type", self.GenericErrorCode.INVALID_VALUE), rule.get("probability", 1.0), rule.get("details", {}))
        return None
    ...
    def _handle_invalid_characters_tokens(self, value: str, rule: Dict[str, Any]) -> Optional[ValidationError]:
        ...
        if invalid_tokens:
            ...
            return self._fail(rule.get("error_type", self.GenericErrorCode.INVALID_FORMAT), rule.get("probability", 0.8), details)
        return None
    ...
    def _handle_material_ambiguous_prefix(self, value: str, rule: Dict[str, Any]) -> Optional[ValidationError]:
        ...
        if len(tokens) > 1 and tokens[0].isalpha() and tokens[1].isalpha():
            ...
            return self._fail(rule.get("error_type", self.GenericErrorCode.INVALID_VALUE), rule.get("probability", 0.6), details)
        return None
    ...
    def _handle_material_prepended_text(self, value: str, rule: Dict[str, Any]) -> Optional[ValidationError]:
        ...
        if len(tokens) > 1 and tokens[0].isalpha() and tokens[1] != ":":
            ...
            return self._fail(rule.get("error_type", self.GenericErrorCode.INVALID_VALUE), rule.get("probability", 0.7), details)
        return None
    ...
    def _handle_material_invalid_hyphen_delimiter(self, value: str, rule: Dict[str, Any]) -> Optional[ValidationError]:
        ...
        for i in range(len(tokens) - 1):
            if tokens[i].endswith('%') and tokens[i + 1] == '-':
                return self._fail(rule.get("error_type", self.GenericErrorCode.INVALID_FORMAT), rule.get("probability", 0.8), rule.get("details", {}))
        return None
    ...
    def _handle_material_malformed_token(self, value: str, rule: Dict[str, Any]) -> Optional[ValidationError]:
        ...
        for i in range(len(tokens) - 2):
            ...
            if is_num and is_word and is_percent:
                ...
                return self._fail(rule.get("error_type", self.GenericErrorCode.INVALID_FORMAT), rule.get("probability", 0.9), details)
        return None
    ...
    def _handle_token_duplicate_adjacent(self, value: str, rule: Dict[str, Any]) -> Optional[ValidationError]:
        ...
        for i in range(len(tokens) - 1):
            if tokens[i].isalpha() and tokens[i] == tokens[i + 1]:
                ...
                return self._fail(rule.get("error_type", self.GenericErrorCode.INVALID_VALUE), rule.get("probability", 0.9), details)
        return None
    ...
    def _handle_season_validator(self, value: Any, rule: Dict[str, Any]) -> Optional[ValidationError]:
        ...
        if pd.isna(value) or value == "":
            return self._fail(rule.get("missing_error_type", "MISSING_VALUE"), rule.get("missing_probability", 1.0), {})
        # Type
        if not isinstance(value, str):
            return self._fail(rule.get("type_error_type", "INVALID_TYPE"), rule.get("type_probability", 1.0), {"expected": "string", "received": str(type(value))})
        parts = value.strip().split(" ", 1)
        if len(parts) != 2:
            return self._fail(rule.get("format_error_type", "INVALID_FORMAT"), rule.get("format_probability", 0.95), {"value": value, "expected_format": "YYYY Season"})
        year_str, season_name = parts
        if not year_str.isdigit() or len(year_str) != 4:
            return self._fail(rule.get("year_error_type", "INVALID_YEAR"), rule.get("year_probability", 0.9), {"year": year_str})
        year = int(year_str)
        ...
        if year < int(rule.get("min_year", 1900)) or year > current_year + max_offset:
            ...
            return self._fail(rule.get("year_error_type", "INVALID_YEAR"), prob, {"year": year, "min_year": int(rule.get("min_year", 1900)), "max_year": current_year + max_offset})
        ...
        if season_name not in valid_seasons:
            return self._fail(rule.get("season_name_error_type", "INVALID_SEASON_NAME"), rule.get("season_name_probability", 0.9), {"season": season_name, "valid_seasons": valid_seasons})
        return None

    # Category specific helper
    def _handle_html_tags_forbidden(self, value: str, rule: Dict[str, Any]) -> Optional[ValidationError]:
        if re.search(r"<[^>]+>", value):
            return self._fail(rule.get("error_type", "HTML_TAGS"), rule.get("probability", 0.98), {"category": value})
        return None
    ...
    def _handle_special_characters_forbidden(self, value: str, rule: Dict[str, Any]) -> Optional[ValidationError]:
        ...
        if re.search(rule.get("forbidden_regex", r"[^\w\s\-]"), value):
            return self._fail(rule.get("error_type", "SPECIAL_CHARACTERS"), rule.get("probability", 0.9), {"value": value})
        return None
    ...
    def _handle_camelcase_merged_words(self, value: str, rule: Dict[str, Any]) -> Optional[ValidationError]:
        if re.search(r"[a-z][A-Z]", value):
            ...
            return self._fail(rule.get("error_type", "MERGED_WORDS"), rule.get("probability", 0.85), details)
        return None
    ...
    def _handle_dashes_instead_of_slashes(self, value: str, rule: Dict[str, Any]) -> Optional[ValidationError]:
        if " - " in value:
            ...
            return self._fail(rule.get("error_type", "INVALID_FORMAT"), rule.get("probability", 0.8), details)
        return None
    ...
    def _handle_starts_with(self, value: str, rule: Dict[str, Any]) -> Optional[ValidationError]:
        ...
        if prefix and not value.startswith(prefix):
            return self._fail(rule.get("error_type", "CONTAINS_PREPENDED_TEXT"), rule.get("probability", 0.9), rule.get("details", {}))
        return None
    ...
    def _handle_contains_both(self, value: str, rule: Dict[str, Any]) -> Optional[ValidationError]:
        ...
        if substr1 in value and substr2 in value:
            return self._fail(rule.get("error_type", "CONTAINS_APPENDED_TEXT"), rule.get("probability", 0.9), rule.get("details", {}))
        return None

    def _handle_contains_multiple_spaces(self, value: str, rule: Dict[str, Any]) -> Optional[ValidationError]:
        if "  " in value:
            return self._fail(rule.get("error_type", "CONTAINS_MULTIPLE_SPACES"), rule.get("probability", 0.85), rule.get("details", {}))
        return None

    def _handle_contains_html(self, value: str, rule: Dict[str, Any]) -> Optional[ValidationError]:
        if re.search(r"<.*?>", value):
            return self._fail(rule.get("error_type", "CONTAINS_HTML"), rule.get("probability", 0.98), rule.get("details", {}))
        return None
    ...
    def _handle_contains_emoji(self, value: str, rule: Dict[str, Any]) -> Optional[ValidationError]:
        ...
        if emoji_pattern.search(value):
            return self._fail(rule.get("error_type", "CONTAINS_EMOJI"), rule.get("probability", 0.95), rule.get("details", {}))
        return None
    ...
    def _handle_contains_disallowed_symbol(self, value: str, rule: Dict[str, Any]) -> Optional[ValidationError]:
        ...
        if symbol and symbol in value:
            ...
            return self._fail(rule.get("error_type", "CONTAINS_DISALLOWED_SYMBOLS"), rule.get("probability", 0.9), details)
        return None
    ...
    def _handle_invalid_delimiter(self, value: str, rule: Dict[str, Any]) -> Optional[ValidationError]:
        if ';' in value or (' - ' in value and 'NIET' in value):
            return self._fail(rule.get("error_type", "INVALID_DELIMITER"), rule.get("probability", 0.9), rule.get("details", {}))
        return None

    def _handle_missing_instruction(self, value: str, rule: Dict[str, Any]) -> Optional[ValidationError]:
        if '. .' in value or '..' in value:
            return self._fail(rule.get("error_type", "MISSING_INSTRUCTION"), rule.get("probability", 0.9), rule.get("details", {}))
        return None
    ...
    def _handle_temperature_required(self, value: str, rule: Dict[str, Any]) -> Optional[ValidationError]:
        ...
        if not re.search(pattern, value):
            ...
            return self._fail(rule.get("error_type", "INVALID_TEMPERATURE_FORMAT"), rule.get("probability", 0.9), details)
        return None
    ...
    def _handle_uppercase_instructions(self, value: str, rule: Dict[str, Any]) -> Optional[ValidationError]:
        ...
        for part in parts:
            if part and "WASSEN OP MAX" not in part and part != part.upper():
                ...
                return self._fail(rule.get("error_type", "INCORRECT_CAPITALIZATION"), rule.get("probability", 0.85), details)
        return None
        ...
        for part in parts:
            if part not in allowed:
                if not re.fullmatch(r"WASSEN OP MAX \d{2}°C", part):
                    return self._fail(rule.get("error_type", "UNKNOWN_INSTRUCTION"), rule.get("probability", 0.8), {"instruction": part})
        return None
